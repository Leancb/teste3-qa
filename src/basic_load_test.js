...
// ===== Opções (cenários dinâmicos por ENV) =====
// Use RUN_SMOKE=1/0 e RUN_LOAD=1/0 para habilitar/desabilitar cada cenário.
// Ex.: apenas smoke -> RUN_LOAD=0 RUN_SMOKE=1
const RUN_SMOKE = (__ENV.RUN_SMOKE ? ? "1") !== "0"; // default ON
const RUN_LOAD = (__ENV.RUN_LOAD ? ? "1") !== "0"; // default ON
    ...
    group("list posts", () => {
        ...
        const ok1 = check(res, {
            ...
            "GET /posts é array não-vazio": (r) => {
                try { const data = r.json(); return Array.isArray(data) && data.length > 0; } catch (_) { return false; }
            },
        });
        ok_rate.add(ok1);
        if (!ok1) errors.add(1);
    ...
    group("get single post", () => {
        ...
        const ok2 = check(res, {
            ...
            "GET /posts/{id} tem id": (r) => {
                try { const data = r.json(); return data && typeof data.id !== "undefined"; } catch (_) { return false; }
            },
        });
        ok_rate.add(ok2);
        if (!ok2) errors.add(1);
...
// ===== Relatórios (summary) =====
// Observação: o k6 NÃO cria diretórios. Garanta a pasta 'reports/'
// antes de rodar (ex.: via npm pre-script ou no CI).
